...
use embassy_rp::pio::{
    Common, Config, Direction, Instance, Irq, PioPin, ShiftDirection, StateMachine,
};
use fixed::FixedU32;
    ...
    PIO: Instance,
{
    ...
    pub async fn write(&mut self, write: &[u32]) -> u32 {
        ...
        let mut status = 0;
        #[cfg(feature = "sequential-dma-probe")]
        {
            self.sm.tx().dma_push(&mut self.dma_tx, write, false).await;
            self.sm
                .rx()
                .dma_pull(&mut self.dma_rx, slice::from_mut(&mut status), false)
                .await;
        }
        #[cfg(not(feature = "sequential-dma-probe"))]
        {
    ...
    async fn read_transaction(&mut self, cmd: u32, read: &mut [u32]) -> u32 {
        self.sm.set_enable(false);
        let write_bits = 31;
        ...
        #[cfg(feature = "sequential-dma-probe")]
        {
            self.sm
                .tx()
                .dma_push(&mut self.dma_tx, slice::from_ref(&cmd), false)
                .await;
            self.sm.rx().dma_pull(&mut self.dma_rx, read, false).await;
        }
        #[cfg(not(feature = "sequential-dma-probe"))]
        {
            let (rx, tx) = self.sm.rx_tx();
    ...
    PIO: Instance,
{
    ...
    async fn cmd_read(&mut self, write: u32, read: &mut [u32]) -> u32 {
        self.cs.set_low();
        let status = self.read_transaction(write, read).await;
        self.cs.set_high();
        status
    }
